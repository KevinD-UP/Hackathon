...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create frontend-6e7f
  fly apps create frontend-6e7f-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app frontend-6e7f
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app frontend-6e7f-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'frontend-6e7f-staging' does not match app name in config file 'frontend-6e7f'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name frontend-6e7f-db
  fly postgres attach --app frontend-6e7f frontend-6e7f-db

  fly postgres create --name frontend-6e7f-staging-db
  fly postgres attach --app frontend-6e7f-staging frontend-6e7f-staging-db
  ```
